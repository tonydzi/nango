...
describe('deployBundle instances', () => {
    ...
    it('commits instances before scheduling, fails deployment on HTTP error, and retries unchanged functions', async () => {
        const ctx = await setup();
        ctx.orchestrator.scheduleFunctions.mockImplementationOnce(async (functions) => {
            // Read on a separate connection: these rows must already be committed.
            expect((await ctx.instances()).map((row) => row.id)).toEqual(functions.map(({ instance }) => instance.id));
            return Err(new Error('orchestrator unavailable'));
        });
    ...
    it('keeps instance deletion committed when orchestrator deletion fails', async () => {
        const ctx = await setup();
        (await ctx.deploy([scheduled])).unwrap();
        ctx.orchestrator.deleteFunctionSchedules.mockImplementationOnce(async () => {
            expect((await ctx.instances())[0]?.deleted_at).toBeInstanceOf(Date);
            return Err(new Error('orchestrator unavailable'));
        });
        expect((await ctx.deploy([])).isErr()).toBe(true);
        expect((await ctx.instances())[0]?.deleted_at).toBeInstanceOf(Date);
    });
